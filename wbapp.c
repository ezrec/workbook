...
static IPTR WBAppWorkbench(Class *cl, Object *obj, Msg msg)
{
    ...
    if (RegisterWorkbench(my->AppPort)) {
		...
		const char *openName = wbhm->wbhm_Data.Open.Name;
                ReplyMsg((APTR)wbhm);
		...
		ULONG imCode = im->Code;
		struct Window *imIDCMPWindow = im->IDCMPWindow;
		GT_ReplyIMsg(im);
                ...
                case IDCMP_CLOSEWINDOW:
                    /* Dispose the window */
                    wbCloseWindow(cl, obj, imIDCMPWindow);
                    break;
                case IDCMP_REFRESHWINDOW:
                    /* call WBWM_REFRESH on the window */
                    wbRefreshWindow(cl, obj, imIDCMPWindow);
                    break;
                case IDCMP_NEWSIZE:
                    /* call WBWM_NEWSIZE on the window */
                    wbNewSizeWindow(cl, obj, imIDCMPWindow);
                    break;
                case IDCMP_MENUPICK:
                    done = wbMenuPick(cl, obj, imIDCMPWindow, imCode);
                    break;
                case IDCMP_INTUITICKS:
                    wbIntuiTick(cl, obj, imIDCMPWindow);
                    break;
                }
            }
        }
